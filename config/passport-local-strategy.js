...
passport.deserializeUser(async function(id, done) {
    ...
    if (!userId) {
        console.log("Error in passport_local_deserializer");
        return;
    }
...
passport.checkAuthentication = function(req, res, next) {
    // if the user is signed in then pass on the request to the
...
passport.setAuthenticatedUser = function(req, res, next) {
    if (req.isAuthenticated()) {
        // req.user contains current signed in user from the session
